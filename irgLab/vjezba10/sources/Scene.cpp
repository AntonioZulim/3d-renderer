...
glm::vec3 Transform::getFrontVec() {
	return mFront;
}
...
// ---------- Light ----------
Light::Light(Shader* shader, bool showIcon) {
	mIcon = new Curve(shader);
	updateIcon();
	mShowIcon = showIcon;
}

void Light::updateIcon() {
	glm::vec3 pos = getPosition();
	mIcon->clearPoints();
	mIcon->addPoint(pos+glm::vec3(-0.05, 0, 0));
	mIcon->addPoint(pos+glm::vec3(0, 0.05, 0));
	mIcon->addPoint(pos+glm::vec3(0.05, 0, 0));
	mIcon->addPoint(pos+glm::vec3(0, -0.05, 0));
	mIcon->addPoint(pos+glm::vec3(-0.05, 0, 0));
	mIcon->addRenderable(BezierBuilder::getControlPolygon(mIcon->aproxCtrlPoints, glm::vec3(1, 1, 0)));
}
void Light::globalMove(glm::vec3 delta) {
	Transform::globalMove(delta);
	updateIcon();
}
void Light::localMove(glm::vec3 delta) {
	Transform::localMove(delta);
	updateIcon();
}
...
// ---------- Spotlight ----------
void Spotlight::initDepthmap(unsigned int index) {
...
glm::mat4 Spotlight::getViewMatrix() {
	return glm::lookAt(getPosition(), getPosition() + getFrontVec(), getUpVec());
}

glm::mat4 Spotlight::getPerspectiveMatrix() {
	return glm::perspective(mCutoff * 2, 1.0f, 0.5f, 10.0f);
}
...
void Object::render(glm::mat4& perspectiveMatrix, glm::mat4& viewMatrix, glm::vec3& eye, std::vector<Light*>& lights) {
	...
	// svjetlo
	mShader->setUniform("numLights", (unsigned int)lights.size());
	for (int i = 0; i<lights.size(); i++) {
		std::string base = "lights[" + std::to_string(i) + "]";
		mShader->setUniform(base + ".position", lights[i]->getPosition());
		mShader->setUniform(base + ".intensity", lights[i]->mIntensity);
		mShader->setUniform(base + ".ambientIntensity", lights[i]->mAmbientIntensity);

		Spotlight* sl = dynamic_cast<Spotlight*>(lights[i]);
		if (sl) {
			glActiveTexture(GL_TEXTURE0 + sl->mTextureIndex);
			glBindTexture(GL_TEXTURE_2D, sl->mShadowMap);
			mShader->setUniform("shadowMaps[" + std::to_string(i) + "]", (int) sl->mTextureIndex);

			mShader->setUniform(base + ".direction", sl->getFrontVec());
			mShader->setUniform(base + ".cutoff", glm::cos(sl->mCutoff));
			mShader->setUniform("lightViewMat[" + std::to_string(i) + "]", sl->getViewMatrix(), false);
			mShader->setUniform("lightPerspectiveMat[" + std::to_string(i) + "]", sl->getPerspectiveMatrix(), false);
		}
		else {
			mShader->setUniform(base + ".direction", glm::vec3(0,0,0));
			mShader->setUniform(base + ".cutoff", -1.0f);
		}
	}
...
void Object::renderDepth(glm::mat4& perspectiveMatrix, glm::mat4& viewMatrix, Shader* shadowShader) {
	glm::mat4 modelMatrix = getModelMatrix();
	shadowShader->use();

	// matrice
	shadowShader->setUniform("modelMat", modelMatrix, false);
	shadowShader->setUniform("viewMat", viewMatrix, false);
	shadowShader->setUniform("perspectiveMat", perspectiveMatrix, false);

	for (auto renderable : mRenderables) {
		renderable->draw(shadowShader);
	}
}
...
// ---------- Renderer ----------
Renderer::Renderer(Shader* shadowShader) {
	InputManager::register_movable(&mCamera);
	InputManager::movable_candidates.push_back(&mCamera);
...
void Renderer::render() {
	for (Light* light : mLights) {
		...
		if (spotlight) {
			spotlight->calculateShadowMap();
			glm::mat4 perspectiveMat = spotlight->getPerspectiveMatrix();
			glm::mat4 viewMat = spotlight->getViewMatrix();

			for (Object* obj : mObjects) {
				obj->renderDepth(perspectiveMat, viewMat, mShadowShader);
			}
		}
	}
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	...
	glm::mat4 perspectiveMat = mCamera.getPerspectiveMatrix(InputManager::width, InputManager::height);
	glm::mat4 viewMat = mCamera.getViewMatrix();
	glm::vec3 eye = mCamera.getPosition();

	for (Object* obj : mObjects) {
		obj->render(perspectiveMat, viewMat, eye, mLights);
	}
	for (Light* light : mLights) {
		if (light->mShowIcon) {
			light->mIcon->render(perspectiveMat, viewMat, eye, mLights);
		}
	}
}
